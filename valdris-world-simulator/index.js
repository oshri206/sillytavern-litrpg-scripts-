...
const rumorSystem = new RumorSystem(stateManager, timeSystem, eventSystem, factionSystem);
...
function createPanel() {
    ...
    container.innerHTML = `
        ...
        <div class="valdris-panel__body">
            ...
            <div class="vws-news-panel">
                <div class="vws-section-header">
                    <span>📰 News & Rumors</span>
                    <span class="vws-news-count" data-role="news-count">0 new</span>
                    <button class="vws-expand-btn" data-action="toggle-news">▼</button>
                </div>
                <div class="vws-news-content hidden" data-role="news-content">
                    <div class="vws-news-feed" data-role="news-feed"></div>
                    <div class="vws-news-buttons">
                        <button data-action="view-news">📋 All News</button>
                        <button data-action="hear-rumors">🍺 Hear Rumors</button>
                        <button data-action="create-rumor">➕ Create Rumor</button>
                    </div>
                </div>
            </div>
        </div>
        <div class="valdris-settings hidden">
            ...
            <div class="valdris-settings__section">
                <div class="valdris-settings__subtitle">News & Rumors</div>
                <label class="valdris-checkbox">
                    <input type="checkbox" data-setting="rumorsEnabled" /> Enable news system
                </label>
                <label class="valdris-checkbox">
                    <input type="checkbox" data-setting="rumorsAuto" /> Auto-generate from events
                </label>
                <label class="valdris-checkbox">
                    <input type="checkbox" data-setting="rumorsSpread" /> Simulate news spreading
                </label>
                <label class="valdris-checkbox">
                    <input type="checkbox" data-setting="rumorsMutate" /> Allow rumor mutation
                </label>
                <label class="valdris-checkbox">
                    <input type="checkbox" data-setting="rumorsNotify" /> Show news notifications
                </label>
                <label class="valdris-checkbox">
                    <input type="checkbox" data-setting="rumorsInject" /> Include news in prompts
                </label>
            </div>
            <div class="valdris-settings__section">
        ...
        <div class="vws-modal vws-news-modal hidden" data-role="news-modal">
            <div class="vws-modal-content large">
                <div class="vws-modal-header">
                    <h3>📰 News & Rumors</h3>
                    <button class="vws-modal-close" data-action="close-news-modal">×</button>
                </div>
                <div class="vws-modal-body" data-role="news-modal-body"></div>
            </div>
        </div>
    `;
    return container;
...
function handlePanelEvents(panel) {
    panel.addEventListener('click', (event) => {
        ...
        if (action === 'toggle-news') {
            panel.querySelector('[data-role="news-content"]').classList.toggle('hidden');
        }
        if (action === 'view-news') {
            openNewsModal(panel, 'all');
        }
        if (action === 'hear-rumors') {
            rumorSystem.generateTavernRumors(3);
            updateNewsUI(panel);
        }
        if (action === 'create-rumor') {
            const text = prompt('Enter rumor text:');
            if (text) {
                rumorSystem.createRumor(text, 'rumor', 'tavern');
                updateNewsUI(panel);
            }
        }
        if (action === 'close-news-modal') {
            panel.querySelector('[data-role="news-modal"]').classList.add('hidden');
        }
    });
    ...
    panel.querySelector('[data-setting="rumorsEnabled"]').addEventListener('change', (event) => {
        const rumors = stateManager.getSection('rumors');
        stateManager.updateSection('rumors', { settings: { ...rumors.settings, enabled: event.target.checked } });
        updateNewsUI(panel);
    });

    panel.querySelector('[data-setting="rumorsAuto"]').addEventListener('change', (event) => {
        const rumors = stateManager.getSection('rumors');
        stateManager.updateSection('rumors', { settings: { ...rumors.settings, autoGenerateFromEvents: event.target.checked } });
    });

    panel.querySelector('[data-setting="rumorsSpread"]').addEventListener('change', (event) => {
        const rumors = stateManager.getSection('rumors');
        stateManager.updateSection('rumors', { settings: { ...rumors.settings, spreadSimulation: event.target.checked } });
    });

    panel.querySelector('[data-setting="rumorsMutate"]').addEventListener('change', (event) => {
        const rumors = stateManager.getSection('rumors');
        stateManager.updateSection('rumors', { settings: { ...rumors.settings, rumorMutation: event.target.checked } });
    });

    panel.querySelector('[data-setting="rumorsNotify"]').addEventListener('change', (event) => {
        const rumors = stateManager.getSection('rumors');
        stateManager.updateSection('rumors', { settings: { ...rumors.settings, showNewsNotifications: event.target.checked } });
    });

    panel.querySelector('[data-setting="rumorsInject"]').addEventListener('change', (event) => {
        const rumors = stateManager.getSection('rumors');
        stateManager.updateSection('rumors', { settings: { ...rumors.settings, injectNewsIntoPrompt: event.target.checked } });
    });
}
...
function hookSillyTavernEvents() {
    ...
    window.eventSource.on(window.event_types.GENERATE_BEFORE_COMBINE_PROMPTS, (data) => {
        ...
        if (rumors?.settings?.enabled && rumors?.settings?.injectNewsIntoPrompt) {
            const newsContext = rumorSystem.getNewsForPrompt();
            if (newsContext) {
                contextLines.push(`[News & Rumors: ${newsContext}]`);
            }
        }
...
function updateNewsUI(panel) {
    const rumorState = stateManager.getSection('rumors');
    if (!rumorState) {
        return;
    }
    const settings = panel.querySelector('.valdris-settings');
    settings.querySelector('[data-setting="rumorsEnabled"]').checked = rumorState.settings.enabled;
    settings.querySelector('[data-setting="rumorsAuto"]').checked = rumorState.settings.autoGenerateFromEvents;
    settings.querySelector('[data-setting="rumorsSpread"]').checked = rumorState.settings.spreadSimulation;
    settings.querySelector('[data-setting="rumorsMutate"]').checked = rumorState.settings.rumorMutation;
    settings.querySelector('[data-setting="rumorsNotify"]').checked = rumorState.settings.showNewsNotifications;
    settings.querySelector('[data-setting="rumorsInject"]').checked = rumorState.settings.injectNewsIntoPrompt;

    const unread = panel.querySelector('[data-role="news-count"]');
    const newCount = rumorSystem.getAllNews().filter((item) => !item.playerHasHeard).length;
    unread.textContent = `${newCount} new`;

    const feed = panel.querySelector('[data-role="news-feed"]');
    feed.innerHTML = '';
    if (!rumorState.settings.enabled) {
        feed.innerHTML = '<div class="vws-empty-state">News system disabled.</div>';
        return;
    }
    const items = rumorSystem.getAllNews().slice(-5).reverse();
    if (!items.length) {
        feed.innerHTML = '<div class="vws-empty-state">No news yet.</div>';
        return;
    }
    items.forEach((news) => {
        const type = rumorSystem.NEWS_TYPES[news.type];
        const item = document.createElement('div');
        item.className = 'news-item';
        item.dataset.importance = news.importance;
        item.dataset.type = news.type;
        item.innerHTML = `
            <span class="news-icon">${type?.icon || '📰'}</span>
            <div class="news-content">
                <span class="news-text">${news.currentText}</span>
                <span class="news-meta">${type?.name || news.type} • ${news.gameDate}</span>
            </div>
        `;
...
function openNewsModal(panel, view) {
    const modal = panel.querySelector('[data-role="news-modal"]');
    const body = panel.querySelector('[data-role="news-modal-body"]');
    body.innerHTML = '';
    const newsItems = view === 'unheard'
        ? rumorSystem.getAllNews().filter((item) => !item.playerHasHeard)
        : rumorSystem.getAllNews();
    if (!newsItems.length) {
        body.innerHTML = '<div class="vws-empty-state">No news available.</div>';
    } else {
        newsItems.forEach((news) => {
            ...
            detail.innerHTML = `
                <div class="news-header">
                    <span class="news-type-badge ${news.type}">${type?.icon || '📰'} ${type?.name || news.type}</span>
                    <span class="news-importance ${news.importance}">${news.importance}</span>
                </div>
                <div class="news-body">
                    <p class="news-text">${news.currentText}</p>
                    ${news.mutationCount ? `<div class="news-accuracy-warning">⚠️ Distorted (${news.mutationCount} mutations)</div>` : ''}
                    ${news.isFalse ? '<div class="news-false-warning">❌ This rumor is false.</div>' : ''}
                </div>
                <div class="news-footer">
                    <div class="news-meta-grid">
                        <div class="meta-item"><span class="meta-label">Origin:</span><span class="meta-value">${news.originLocation}</span></div>
                        <div class="meta-item"><span class="meta-label">Date:</span><span class="meta-value">${news.gameDate}</span></div>
                        <div class="meta-item"><span class="meta-label">Spread:</span><span class="meta-value">${news.currentSpread.length} locations</span></div>
                        ${news.playerHasHeard ? `<div class="meta-item"><span class="meta-label">Heard via:</span><span class="meta-value">${news.playerHeardFrom}</span></div>` : ''}
                    </div>
                </div>
            `;
            body.appendChild(detail);
...
function showNewsNotification(news) {
    const settings = stateManager.getSection('rumors').settings;
    if (!settings.showNewsNotifications) {
        return;
    }
    const type = rumorSystem.NEWS_TYPES[news.type];
    const toast = document.createElement('div');
    toast.className = 'vws-news-toast';
    toast.style.setProperty('--news-color', type?.icon ? '#4a9eff' : '#666');
    toast.innerHTML = `
        <div class="toast-header">
            <span class="toast-type">${type?.icon || '📰'} ${type?.name || news.type}</span>
        </div>
        <div class="toast-text">${news.currentText}</div>
    `;
    document.body.appendChild(toast);
    setTimeout(() => toast.remove(), 4000);
}
...
async function initialize() {
    ...
    window.addEventListener('vws-news', (event) => {
        if (event?.detail) {
            showNewsNotification(event.detail);
            updatePanel(panel);
        }
    });
    updatePanel(panel);
